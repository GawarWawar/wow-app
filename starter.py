...
from utils.tools import read_columns_of_the_csv_to_DF, read_the_file_to_DF, find_item_in_DataFrame
...
def give_all_aviable_guild_stats(name):
    guld_name = escape(name)
    df_to_work_with_guild = read_the_file_to_DF(dynamic_database["guilds_table"])
    df_to_work_with_characters = read_the_file_to_DF(dynamic_database["characters_table"])
    df_to_send = 1
    return (guld_name)
